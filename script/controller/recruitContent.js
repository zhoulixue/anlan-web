angular.module('rootApp.recruitContent',[
])
.config(function($routeProvider){
	 $routeProvider
        .when('/recruitContent', {
            templateUrl: 'views/recruitContent.html',
            controller: 'recruitContentController'
        })
        .when('/recruitCultureContent', {
            templateUrl: 'views/recruitCultureContent.html',
            controller: 'recruitContentController'
        })
})
.controller('recruitContentController',function ($scope,$http,$location) {
    ...
    $scope.toPreNews = function(preNews){
        if (preNews === undefined || preNews === null || preNews === "") {
            ...
            $location.url('/recruitContent?id=' + preNews.id)
        }
    }
    ...
    $scope.toNextNews = function(nextNews){
        if (nextNews === undefined || nextNews === null || nextNews === "") {
            ...
            $location.url('/recruitContent?id=' + nextNews.id)
        }
    }
})