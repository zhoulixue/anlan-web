angular.module('rootApp.projectContent',[
])
...
.controller('projectContentController',function ($scope,$http,$location) {
    ...
    $scope.toPreNews = function(preNews){
        if (preNews === undefined || preNews === null || preNews === "") {
            ...
            $location.url('/projectContent?id=' + preNews.id)
        }
    }
    ...
    $scope.toNextNews = function(nextNews){
        if (nextNews === undefined || nextNews === null || nextNews === "") {
            ...
            $location.url('/projectContent?id=' + nextNews.id)
        }
    }
})