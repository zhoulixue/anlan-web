angular.module('rootApp.newsHolidayContent',[
])
.config(function($routeProvider){
	 $routeProvider
        .when('/newsHolidayContent', {
            templateUrl: 'views/newsHolidayContent.html',
            controller: 'newsHolidayContentController'
        })
})
.controller('newsHolidayContentController',function ($scope,$http,$location) {
        ...
        .then(function(res){
            for (var i = 0; i < res.data.news.length; i++) {
                if (res.data.news[i].id == $scope.newsId) {
                    ...
                    console.log($scope.currentNews.tail)
                }
            }
    ...
    $scope.toPreNews = function(preNews){
        if (preNews === undefined || preNews === null || preNews === "") {
            ...
            $location.url('/newsHolidayContent?id=' + preNews.id)
        }
    }
    ...
    $scope.toNextNews = function(nextNews){
        if (nextNews === undefined || nextNews === null || nextNews === "") {
            ...
            $location.url('/newsHolidayContent?id=' + nextNews.id)
        }
    }
})